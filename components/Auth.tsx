...
const Auth: React.FC<AuthProps> = ({ onLogin, onBack }) => {
  ...
  const handleGoogleLogin = async () => {
    if (!isFirebaseConfigured) {
      setError({ msg: 'ফায়ারবেস সিস্টেম কনফিগারেশন ত্রুটি। ডেভলপারকে জানান।', code: 'CONFIG_MISSING' });
      return;
    }
    ...
    try {
      ...
      if (err.code === 'auth/api-key-not-valid') message = 'সিস্টেম এপিআই কি-টি সঠিক নয়।';
  ...
  const handleSendMagicLink = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!isFirebaseConfigured) {
      setError({ msg: 'সিস্টেম কনফিগারেশন ত্রুটি। ইমেইল পাঠানো সম্ভব নয়।', code: 'CONFIG_MISSING' });
      return;
    }
      ...
      <div className="w-full max-w-sm">
        ...
        {!isFirebaseConfigured && (
          <div className="mb-6 p-4 bg-orange-50 border border-orange-200 rounded-2xl text-center">
             <p className="text-xs font-black text-orange-700">⚠️ সিস্টেম সেটআপ অসম্পূর্ণ</p>
             <p className="text-[10px] text-orange-600 font-bold mt-1">কোডে ফায়ারবেস কনফিগারেশন পাওয়া যায়নি।</p>
          </div>
        )}
