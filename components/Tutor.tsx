...
const Tutor: React.FC<TutorProps> = ({ user, subject, history, onUpdateHistory, onBack, classLevel, group }) => {
  ...
  // ✅ পিডিএফ ডাউনলোডের সময় স্ক্রিনের হুবহু ফরম্যাট (Heading ও Bold) বজায় রাখার লজিক
  const handleDownloadPDF = (messageText: string) => {
    ...
    // স্ক্রিনের স্টাইল অনুযায়ী এইচটিএমএল (HTML) তৈরি করা হচ্ছে
    const formattedHtml = messageText
      .replace(/\$/g, '')
      .split('\n')
      .map((line) => {
        if (line.trim().startsWith('###')) {
          return `<h2 style="font-size: 18px; font-weight: 800; margin-top: 22px; margin-bottom: 12px; border-left: 4px solid #10b981; padding-left: 10px; color: #059669; font-family: sans-serif;">${line.replace('###', '').trim()}</h2>`;
        }
        // বোল্ড টেক্সট কনভার্ট করা হচ্ছে
        const processedLine = line.replace(/\*\*(.*?)\*\*/g, '<strong style="color: #059669; font-weight: 700;">$1</strong>');
        return `<p style="font-size: 13px; margin-bottom: 10px; line-height: 1.7; color: #334155; font-family: sans-serif;">${processedLine}</p>`;
      })
      .join('');

    element.innerHTML = `
      <div style="font-family: sans-serif; padding: 35px; color: #1e293b; background-color: #ffffff;">
        <div style="border-bottom: 2px solid #10b981; padding-bottom: 12px; margin-bottom: 22px;">
          <h1 style="color: #059669; margin: 0; font-size: 22px; font-weight: 800;">${subject} — লেকচার নোট</h1>
          <p style="color: #64748b; margin: 5px 0 0 0; font-size: 11px;">Generated by Saiyed AI Tutor</p>
        </div>
        <div style="letter-spacing: 0.3px;">${formattedHtml}</div>
      </div>
    `;
