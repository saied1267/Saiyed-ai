...
const Tutor: React.FC<TutorProps> = ({ user, subject, history, onUpdateHistory, onBack, classLevel, group }) => {
  ...
  const handleDownloadPDF = async (messageText: string) => {
    ...
    const formattedHtml = processedText.split('\n').map((line) => {
      if (line.trim().startsWith('###')) {
        return `<h2 style="font-size: 22px; font-weight: 700; margin: 25px 0 15px 0; border-left: 5px solid #10b981; padding-left: 12px; color: #059669; font-family: 'Hind Siliguri', sans-serif; page-break-after: avoid; break-after: avoid;">${line.replace('###', '').trim()}</h2>`;
      }
      const tokens = line.split(/(\$\$.*?\$\$|\$.*?\$)/g);
      const lineHtml = tokens.map((token) => {
        if (token.startsWith('$$') && token.endsWith('$$')) {
            // গণিতের স্টাইলিং ঠিক রাখার জন্য Center alignment ব্যবহার করা হয়েছে
            return win.katex ? `<div style="text-align: center; margin: 15px 0;">${win.katex.renderToString(token.slice(2, -2), { displayMode: true })}</div>` : token;
        }
        if (token.startsWith('$') && token.endsWith('$')) {
            return win.katex ? win.katex.renderToString(token.slice(1, -1), { displayMode: false }) : token;
        }
        return token.replace(/\*\*(.*?)\*\*/g, '<span style="background-color: #ffff00; color: #000; padding: 0 2px; border-radius: 2px;">$1</span>');
      }).join('');
      return `<p style="font-size: 16px; margin-bottom: 15px; line-height: 1.8; color: #1e293b; font-family: 'Hind Siliguri', sans-serif; page-break-inside: avoid; break-inside: avoid;">${lineHtml}</p>`;
    }).join('');
    ...
    // PDF জেনারেটরের মূল HTML-এর ভেতরে KaTeX এর CSS লিংক দেওয়া হলো
    const htmlContent = `
      <!DOCTYPE html>
      <html>
      <head>
      ...
      <body>
        <div style="padding: 30px; background-color: #ffffff; font-family: 'Hind Siliguri', sans-serif; background-image: url('${watermarkUrl}'); background-repeat: repeat; background-size: 600px 600px; -webkit-print-color-adjust: exact; print-color-adjust: exact; min-height: 100vh;">
          <div style="border-bottom: 3px solid #10b981; padding-bottom: 15px; margin-bottom: 30px; position: relative; z-index: 10; page-break-after: avoid; break-after: avoid;">
  ...
  return (
    <div className="fixed inset-0 z-[60] flex flex-col bg-slate-50 dark:bg-[#09090b]">
      ...
      <header className="px-4 py-3.5 flex items-center justify-between border-b border-slate-200 dark:border-zinc-800 bg-white dark:bg-[#09090b] sticky top-0 z-50">
        <div className="flex items-center space-x-3">
          ...
          <div>
            ...
            <p className="text-[11px] text-emerald-600 dark:text-emerald-400 font-bold">সাঈদ AI সক্রিয়</p>
          </div>
        </div>
              ...
              <div className="flex flex-col space-y-3">
                {initialSuggestions.map((s, i) => (
                  <button
                    ...
                    className="p-4 bg-transparent border border-slate-200 dark:border-zinc-800 rounded-xl text-left font-semibold text-slate-700 dark:text-zinc-300 hover:border-emerald-400 dark:hover:border-emerald-600 hover:shadow-md transition duration-200"
                  >
          ...
          {history.map((m, actualIdx) => {
            ...
            return (
              <div key={actualIdx} className={`flex flex-col ${m.role === 'user' ? 'items-end' : 'items-start'} space-y-2`}>
                ...
                <div className={`${
                  m.role === 'user' 
                    ? 'max-w-[85%] bg-transparent border border-slate-300 dark:border-zinc-700 px-5 py-3.5 rounded-2xl text-[15px]' 
                    : 'w-full bg-white dark:bg-zinc-900 border dark:border-zinc-800 shadow-sm p-5 rounded-xl'
                }`}>
                  {m.role === 'model' && actualIdx === history.length - 1 && loading && !m.text ? (
                    ...
                    <div className={`prose dark:prose-invert max-w-none ${m.role === 'user' ? 'text-slate-700 dark:text-zinc-300 font-medium' : 'text-slate-800 dark:text-zinc-200'}`}>
                      {renderText(m.text)}
                    </div>
                  )}
                </div>
                ...
                <div className="flex items-center space-x-2 mt-1">
                  <button
                    ...
                    className={`px-2 py-1 text-xs rounded transition bg-slate-200 dark:bg-zinc-800 hover:bg-slate-300 dark:hover:bg-zinc-700 text-slate-600 dark:text-slate-300`}
                    title="কপি"
                  >
                    ...
                    <div className="flex flex-col gap-2">
                      {followUpQuestions.slice(0, 2).map((question: string, idx: number) => (
                        <button
                          ...
                          className="text-left text-xs px-3 py-2 bg-emerald-50 dark:bg-emerald-950/30 border border-emerald-200 dark:border-emerald-800 rounded-lg text-emerald-700 dark:text-emerald-300 hover:bg-emerald-100 dark:hover:bg-emerald-900/50 transition shadow-sm"
                        >
                    ...
                    <div className="flex flex-wrap gap-2">
                      {aiSuggestions.slice(0, 4).map((suggestion: string, idx: number) => (
                        <button
                          ...
                          className="text-left text-xs px-3 py-1.5 bg-emerald-50 dark:bg-emerald-900/30 border border-emerald-200 dark:border-emerald-800 rounded-lg text-emerald-700 dark:text-emerald-300 hover:bg-emerald-100 dark:hover:bg-emerald-800/50 transition shadow-sm"
                        >
      ...
      <div className="p-4 bg-white dark:bg-[#09090b] border-t dark:border-zinc-800 pb-8">
        <div className="max-w-2xl mx-auto flex items-center bg-white dark:bg-zinc-900 p-1.5 rounded-xl border border-slate-200 dark:border-zinc-800 shadow-sm focus-within:ring-2 focus-within:ring-emerald-500/30 focus-within:border-emerald-500 transition-all">
          <textarea
            rows={1}
            value={input}
          ...
          <button
            ...
            className="p-2.5 bg-emerald-600 text-white rounded-lg disabled:opacity-50 hover:bg-emerald-700 hover:scale-105 active:scale-95 transition-all shadow-md"
          >
            <svg viewBox="0 0 24 24" width="20" height="20" fill="currentColor">
