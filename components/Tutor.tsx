  ...
  const renderText = (text: string) => {
    ...
    return processedText.split('\n').map((line, i) => {
      if (line.trim().startsWith('###')) {
        return <h2 key={i} className="text-[19px] font-bold mt-5 mb-2 text-indigo-600 dark:text-indigo-400 border-l-4 border-indigo-500 pl-3">{line.replace('###', '').trim()}</h2>;
      }
...
const handleDownloadPDF = async (messageText: string) => {
    ...
    const formattedHtml = processedText.split('\n').map((line) => {
      if (line.trim().startsWith('###')) {
        return `<h2 style="font-size: 22px; font-weight: 700; margin: 25px 0 15px 0; border-left: 5px solid #4f46e5; padding-left: 12px; color: #4338ca; font-family: 'Hind Siliguri', sans-serif;">${line.replace('###', '').trim()}</h2>`;
      }
      const tokens = line.split(/(\$\$.*?\$\$|\$.*?\$)/g);
    ...
    const htmlContent = `
      <div style="padding: 30px; background-color: #ffffff; font-family: 'Hind Siliguri', sans-serif; background-image: url('${watermarkUrl}'); background-repeat: repeat; background-size: 600px 600px; -webkit-print-color-adjust: exact; print-color-adjust: exact; min-height: 100vh;">
        <div style="border-bottom: 3px solid #4f46e5; padding-bottom: 15px; margin-bottom: 30px; position: relative; z-index: 10;">
          <h1 style="color: #4338ca; font-size: 24px; margin: 0;">${subject} লেকচার নোট</h1>
  ...
  return (
    <div className="fixed inset-0 z-[60] flex flex-col bg-slate-50 dark:bg-[#09090b]">
      ...
      <header className="px-4 py-3.5 flex items-center justify-between border-b border-slate-200 dark:border-zinc-800 bg-white dark:bg-[#09090b] sticky top-0 z-50">
        <div className="flex items-center space-x-3">
          ...
          <div>
            ...
            <p className="text-[11px] text-indigo-600 dark:text-indigo-400 font-bold">সাঈদ AI সক্রিয়</p>
          </div>
        </div>
              ...
              <div className="flex flex-col space-y-3">
                {initialSuggestions.map((s, i) => (
                  <button
                    ...
                    className="p-4 bg-white dark:bg-zinc-900 border border-slate-200 dark:border-zinc-800 rounded-xl text-left font-semibold text-slate-700 dark:text-zinc-300 hover:border-indigo-400 dark:hover:border-indigo-600 hover:shadow-md dark:hover:bg-zinc-800/70 transition duration-200"
                  >
          ...
          {history.map((m, actualIdx) => {
            ...
            return (
              <div key={actualIdx} className={`flex flex-col ${m.role === 'user' ? 'items-end' : 'items-start'} space-y-2`}>
                ...
                <div className={`${m.role === 'user' ? 'max-w-[85%] bg-gradient-to-br from-indigo-500 to-blue-600 text-white px-5 py-3.5 rounded-2xl rounded-br-sm shadow-md text-[15px]' : 'w-full bg-white dark:bg-zinc-900 border dark:border-zinc-800 shadow-sm p-5 rounded-xl'}`}>
                  {m.role === 'model' && actualIdx === history.length - 1 && loading && !m.text ? (
                    <div className="flex items-center space-x-2 py-3">
                      ...
                      <span className="text-indigo-500 font-bold text-sm">{LOADING_MESSAGES[loadingStep]}</span>
                    </div>
                  ) : (
                    <div className={`prose dark:prose-invert max-w-none ${m.role === 'user' ? 'text-white font-medium' : 'text-slate-800 dark:text-zinc-200'}`}>
                      {renderText(m.text)}
                    </div>
                  )}
                </div>
                ...
                <div className="flex items-center space-x-2 mt-1">
                  <button
                    ...
                    className={`px-2 py-1 text-xs rounded transition ${m.role === 'user' ? 'bg-indigo-500/20 hover:bg-indigo-500/40 text-indigo-700 dark:text-indigo-300' : 'bg-slate-200 dark:bg-zinc-800 hover:bg-slate-300 dark:hover:bg-zinc-700 text-slate-600 dark:text-slate-300'}`}
                    title="কপি"
                  >
                  ...
                  {m.role === 'model' && m.text && (
                    <button
                      ...
                      className="flex items-center space-x-1 px-2 py-1 text-xs bg-indigo-600 hover:bg-indigo-700 text-white rounded transition shadow-sm"
                      title="PDF ডাউনলোড"
                    >
                    ...
                    <div className="flex flex-col gap-2">
                      {followUpQuestions.slice(0, 2).map((question: string, idx: number) => (
                        <button
                          ...
                          className="text-left text-xs px-3 py-2 bg-blue-50 dark:bg-blue-950/30 border border-blue-200 dark:border-blue-800 rounded-lg text-blue-700 dark:text-blue-300 hover:bg-blue-100 dark:hover:bg-blue-900/50 transition shadow-sm"
                        >
                ...
                {m.role === 'model' && m.text && aiSuggestions.length > 0 && (
                  <div className="w-full mt-3 space-y-2 animate-in slide-in-from-bottom-2">
                    <p className="text-xs font-bold text-indigo-600 dark:text-indigo-400">✨ সাজেশন্স:</p>
                    <div className="flex flex-wrap gap-2">
                      {aiSuggestions.slice(0, 4).map((suggestion: string, idx: number) => (
                        <button
                          key={idx}
                          onClick={() => handleSend(suggestion)}
                          className="text-left text-xs px-3 py-1.5 bg-indigo-50 dark:bg-indigo-900/30 border border-indigo-200 dark:border-indigo-800 rounded-lg text-indigo-700 dark:text-indigo-300 hover:bg-indigo-100 dark:hover:bg-indigo-800/50 transition shadow-sm"
                        >
      ...
      <div className="p-4 bg-white dark:bg-[#09090b] border-t dark:border-zinc-800 pb-8">
        <div className="max-w-2xl mx-auto flex items-center bg-white dark:bg-zinc-900 p-1.5 rounded-xl border border-slate-200 dark:border-zinc-800 shadow-sm focus-within:ring-2 focus-within:ring-indigo-500/30 focus-within:border-indigo-500 transition-all">
          <textarea
            rows={1}
            value={input}
          ...
          />
          {/* Send Button Design Fix (Premium iOS Style) */}
          <button
            onClick={() => handleSend()}
            disabled={!input.trim() || loading}
            className="p-2.5 bg-gradient-to-r from-indigo-500 to-blue-600 text-white rounded-lg disabled:opacity-50 hover:shadow-lg hover:scale-105 active:scale-95 transition-all shadow-md"
          >
            <svg viewBox="0 0 24 24" width="20" height="20" fill="currentColor">
