...
const Tutor: React.FC<TutorProps> = ({ user, subject, history, onUpdateHistory, onBack, classLevel, group }) => {
  ...
  const handleDownloadPDF = async (messageText: string) => {
    ...
    const formattedHtml = processedText.split('\n').map((line) => {
      if (line.trim().startsWith('###')) {
        // হেডিংয়ের পরে যাতে পৃষ্ঠা না ভাঙে (break-after: avoid)
      ...
      // অনুচ্ছেদের মাঝখানে যাতে পৃষ্ঠা না ভাঙে (break-inside: avoid)
    ...
    // অপশনে pagebreak এবং margin আপডেট করা হয়েছে
    const opt = {
      ...
      pagebreak: { mode: ['avoid-all', 'css', 'legacy'] } // পৃষ্ঠা ভাঙা প্রতিরোধ করবে
    };
