...
const Tutor: React.FC<TutorProps> = ({ user, subject, history, onUpdateHistory, onBack, classLevel, group }) => {
  ...
  const [loadingStep, setLoadingStep] = useState(0); // একের পর এক লেখা পরিবর্তনের স্টেট
  const scrollRef = useRef<HTMLDivElement>(null);

  // Math Rendering এবং Scrolling Effect
  useEffect(() => {
  ...
  // লোডিং টেক্সট একের পর এক পরিবর্তন করার টাইমার লজিক
  useEffect(() => {
    let interval: NodeJS.Timeout;
    if (loading) {
      setLoadingStep(0); // প্রতিবার নতুন মেসেজে শুরু থেকে শুরু হবে
      interval = setInterval(() => {
        setLoadingStep((prev) => (prev < 2 ? prev + 1 : 0)); // ৩টি মেসেজের মধ্যে লুপ হবে
      }, 2000); // প্রতি ২ সেকেন্ড পর পর পরিবর্তন হবে
    } else {
      setLoadingStep(0);
    }
  ...
  const handleSend = async (text?: string) => {
    ...
    try {  
      ...
      console.error(e);  
      onUpdateHistory([...newHistory, { ...aiPlaceholder, text: "⚠️ এআই সার্ভারে সংযোগ বিচ্ছিন্ন হয়েছে। সাঈদ এর সাথে যোগাযোগ করুন।" }]);  
    }  
          ...
          {history.map((m, i) => (  
            <div key={i} className={`flex flex-col space-y-4 ${m.role === 'user' ? 'items-end' : 'items-start'}`}>  
              <div className={`${m.role === 'user' ? 'max-w-[85%] bg-slate-100 dark:bg-white/10 dark:text-white rounded-3xl px-6 py-4 shadow-sm' : 'w-full'}`}>  
                ...
                {/* লোডিং অ্যানিমেশন ও পরিবর্তনশীল টেক্সট সেকশন */}
                {m.role === 'model' && i === history.length - 1 && loading && !m.text ? (  
                   <div className="py-8 space-y-4">  
                     ...
                     <p className="text-[14px] font-black text-emerald-600 dark:text-emerald-400 tracking-tight uppercase animate-pulse min-h-[20px]">
                       {loadingStep === 0 && "Saiyed AI গভীরভাবে বিশ্লেষণ করছে..."}
                       {loadingStep === 1 && "Saiyed AI চিন্তা করছে..."}
                       {loadingStep === 2 && "Saiyed AI উত্তর তৈরি করছে..."}
                     </p>  
                   </div>  
                ) : (  
