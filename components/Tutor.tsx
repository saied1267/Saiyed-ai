...
const Tutor: React.FC<TutorProps> = ({ user, subject, history, onUpdateHistory, onBack, classLevel, group }) => {
  ...
  const handleDownloadPDF = async (messageText: string) => {
    ...
    // ডায়াগোনাল ওয়াটারমার্ক SVG জেনারেট এবং Base64 কনভার্ট
    const watermarkSvg = `<svg xmlns="http://www.w3.org/2000/svg" width="450" height="400"><text x="225" y="200" fill="rgba(0,0,0,0.035)" font-size="15" font-family="sans-serif" font-weight="bold" transform="rotate(-45 225 200)" text-anchor="middle">Kaisir ahamed Saiyed (Saiyed aI)</text></svg>`;
