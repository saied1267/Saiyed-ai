...
const Tutor: React.FC<TutorProps> = ({ subject, history, onUpdateHistory, onBack, classLevel, group, user }) => {
  ...
  const renderFormattedText = (text: string) => {
    return text.split('\n').map((line, idx) => {
      ...
      return <p key={idx} className="mb-4 text-[14px] font-bold leading-relaxed text-slate-700 dark:text-slate-300">{line}</p>;
    });
  };
          ...
          <div key={i} className={`flex w-full ${m.role === 'user' ? 'justify-end' : 'justify-start'} animate-in slide-in-from-bottom-2`}>
            <div className={`w-full ${m.role === 'user' ? 'max-w-[85%] flex flex-col items-end' : ''}`}>
              <div className={`${m.role === 'user' ? 'bg-slate-100 dark:bg-slate-900 p-5 rounded-[2rem] rounded-tr-sm' : 'w-full'}`}>
                {m.image && <img src={m.image} className="w-full rounded-2xl mb-4 max-h-80 object-cover border dark:border-slate-800" alt="at" />}
      ...
      <div className="px-6 py-6 border-t dark:border-slate-900 bg-white/95 dark:bg-slate-950/95 backdrop-blur-xl pb-10 mt-auto">
        {selectedImage && (
          <div className="relative inline-block mb-4">
            <img src={selectedImage} className="w-20 h-20 object-cover rounded-2xl border-2 border-emerald-500" alt="p" />
