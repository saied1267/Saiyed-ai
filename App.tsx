...
const App: React.FC = () => {
  ...
  // Initial local load from storage
  useEffect(() => {
  ...
  // Persistent Cloud Listener - Restores data on Login
  useEffect(() => {
    ...
    const unsubUser = onSnapshot(userDocRef, (snapshot) => {
      if (snapshot.exists()) {
        ...
        // Sync Histories and persist to localStorage
        ...
        // Ensure name/profile is up to date
  ...
  // Main sync function called when data changes locally
  ...
  return (
    <div className={`flex flex-col fixed inset-0 w-full bg-[#F0F2F5] dark:bg-slate-950 transition-colors ${darkMode ? 'dark' : ''}`}>
      {/* Subtle Sync Line for User */}
      {isSyncing && (
        <div className="absolute top-0 left-0 w-full h-[2px] bg-emerald-500/30 z-[100] overflow-hidden">
      ...
      <main className="flex-1 overflow-y-auto max-w-lg mx-auto w-full px-4 pt-4 scrollbar-hide">
        ...
        {currentView === View.SETTINGS && <Settings user={user} onUpdateInterests={updateInterests} onGoToAuth={() => setCurrentView(View.AUTH)} darkMode={darkMode} setDarkMode={setDarkMode} onResetAll={() => { if (confirm('এটি আপনার সব লোকাল ডেটা মুছে ফেলবে এবং আপনাকে লগআউট করে দিবে। নিশ্চিত তো?')) handleLogout(); }} />}
      </main>
